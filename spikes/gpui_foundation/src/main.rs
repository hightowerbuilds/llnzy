...
use gpui::prelude::*;
use gpui::{
    actions, canvas, div, fill, hsla, point, px, relative, rgb, rgba, size, App, Application,
    Bounds, ClipboardItem, Context, CursorStyle, Element, ElementId, ElementInputHandler, Entity,
    EntityInputHandler, FocusHandle, Focusable, GlobalElementId, KeyBinding, LayoutId, MouseButton,
    MouseDownEvent, MouseMoveEvent, MouseUpEvent, PaintQuad, PathBuilder, Pixels, Point, Render,
...
impl FoundationSpike {
    fn new(cx: &mut Context<Self>) -> Self {
        ...
        let files = (0..80)
            ...
            .collect();
        let prompt_input = cx.new(|cx| TextInput {
            focus_handle: cx.focus_handle(),
            content: "Draft a concise terminal bridge test plan.".into(),
            placeholder: "Type a prompt here...".into(),
            selected_range: 0..0,
    ...
    Application::new().run(|cx: &mut App| {
        ...
        let window = cx
            .open_window(
                ...
                |_, cx| cx.new(FoundationSpike::new),
            )
            .unwrap();
        window
            .update(cx, |view, window, cx| {
                window.focus(&view.prompt_input.focus_handle(cx));
            })
            .unwrap();
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let (display_text, text_color) = if content.is_empty() {
            (input.placeholder.clone(), hsla(0.0, 0.0, 1.0, 0.35))
        } else {
            (content, style.color)
        };
