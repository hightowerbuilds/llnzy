...
use super::editor_paint::{pixel_to_editor_pos, EditorGeometry};
...
#[expect(
    ...
    reason = "pointer input still coordinates egui response, editor state, folding, wrapping, and geometry until an input context struct is introduced"
)]
pub(super) fn handle_editor_pointer_input(
    ...
    if response.hovered() {
        ...
        if scroll_delta != 0.0 {
            let scroll_lines = (-scroll_delta / geometry.line_height).round() as i32;
    ...
    painter.rect_filled(geometry.rect, 0.0, egui::Color32::from_rgb(25, 25, 32));
    let gutter_rect = egui::Rect::from_min_size(
        geometry.rect.left_top(),
        egui::Vec2::new(geometry.gutter_width, geometry.rect.height()),
    );
    painter.rect_filled(gutter_rect, 0.0, egui::Color32::from_rgb(30, 30, 38));

    let handled_gutter_click =
        handle_gutter_click(response, view, foldable_ranges, visible_doc_lines, geometry);
    handle_editor_click(
        response,
        buf,
        view,
...
fn gutter_fold_range_at_pos(
    ...
    if !is_gutter_pos(pos, geometry) {
        return None;
    }

    let visible_idx = visible_index_for_y(pos.y, geometry, scroll_line);
    let doc_line = visible_doc_lines.get(visible_idx).copied()?;
    best_fold_range_starting_at(foldable_ranges, doc_line)
}

fn is_gutter_pos(pos: egui::Pos2, geometry: EditorGeometry) -> bool {
    pos.x < geometry.rect.left() + geometry.gutter_width
}

fn visible_index_for_y(pos_y: f32, geometry: EditorGeometry, scroll_line: usize) -> usize {
    if geometry.line_height <= 0.0 {
        return scroll_line;
    }

    scroll_line + ((pos_y - geometry.rect.top()) / geometry.line_height).max(0.0) as usize
}
...
#[expect(
    clippy::too_many_arguments,
    reason = "click handling still coordinates pointer hit testing and wrapping state until an input context struct is introduced"
)]
fn handle_editor_click(
    ...
    if !editor_pointer_allowed(pos, geometry.rect, handled_gutter_click) {
        return;
    }
    ...
    let (click_line, click_col) = if word_wrap {
        pixel_to_editor_pos_wrapped(WrappedHitTestInput {
            ...
            text_margin: geometry.text_margin,
        ...
        pixel_to_editor_pos(pos, geometry, view.scroll_line, visible_doc_lines, buf)
    };
...
fn handle_editor_drag(
    ...
    if !editor_pointer_allowed(pos, geometry.rect, false) {
        return;
    }
...
mod tests {
    ...
    fn editor_geometry() -> EditorGeometry {
    ...
    fn gutter_hit_testing_excludes_gutter_right_edge() {
        let geometry = editor_geometry();

        assert!(is_gutter_pos(egui::pos2(49.9, 30.0), geometry));
        assert!(!is_gutter_pos(egui::pos2(50.0, 30.0), geometry));
    }
