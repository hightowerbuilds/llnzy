...
use super::settings_state::SettingsUiState;

pub(super) fn render_sketch_mock_preview(ui: &mut egui::Ui, config: &Config, sketch: &SketchState) {
...
pub(super) fn render_code_editor_mock_preview(ui: &mut egui::Ui, config: &Config) {
    let available = ui.available_size();
    let preview_w = available.x.max(1.0);
    let preview_h = available.y.max(1.0);
    let (rect, _) = ui.allocate_exact_size(egui::vec2(preview_w, preview_h), egui::Sense::hover());
...
pub(super) fn render_terminal_mock_preview(
    ui: &mut egui::Ui,
    config: &Config,
