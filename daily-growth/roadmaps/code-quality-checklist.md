...
```sh
find src tests spikes -name '*.rs' -print0 | xargs -0 wc -l | awk '$2 != "total" && $1 > 1000 { print; failed = 1 } END { exit failed }'
```

Regenerate the roadmap skyscraper list:

```sh
find src tests spikes -name '*.rs' -print0 | xargs -0 wc -l | sort -nr | awk '$2 != "total" && $1 >= 1000 { printf "| %s | `%s` |\n", $1, $2 }'
```
