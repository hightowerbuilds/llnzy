...
- [ ] Audit current sidebar move, rename, dirty-file blocking, open-tab remapping, and file watcher paths.
- [ ] Define a `MoveRequest` model that represents source path, destination directory, final path, source kind, and whether the move came from drag/drop or the context menu.
- [ ] Define shared move validation that blocks:
  - dirty open files,
...
- [ ] Define shared move execution that performs the filesystem move immediately so Finder reflects the result without waiting for a refresh cycle.
- [ ] Reuse the shared execution path for sidebar drag/drop.
- [ ] Add a project-root drop target with clear visual feedback.
- [ ] Strengthen folder hover/drop hit testing so users can tell exactly where a file or folder will land.
- [ ] Add visible drop indicators for root, folder, invalid target, and blocked dirty-file target.
- [ ] Add right-click `Move...` for files and folders.
- [ ] Build a searchable folder picker/tree for `Move...`.
- [ ] Include the project root as a first-class destination in the move picker.
- [ ] Disable invalid destinations in the picker instead of allowing a failed move.
- [ ] After a clean open file moves, update any open editor tab path/title to the new path.
- [ ] After a blocked move, leave the file exactly where it was and show a clear status or error message.
- [ ] Add focused unit tests for validation rules that do not require the desktop UI.
- [ ] Manually verify clean file drag/drop into a folder.
- [ ] Manually verify clean file drag/drop into project root.
- [ ] Manually verify dirty open file drag/drop is blocked.
- [ ] Manually verify clean open file right-click `Move...` updates the tab.
- [ ] Manually verify dirty open file right-click `Move...` is blocked.
...
- [ ] Preserve the current styled Stacker prompt editor; the May 3, 2026 test showed the styling is not the Wispr Flow failure.
- [ ] Audit all Stacker command routing paths: keybindings, native menu actions, command palette actions, toolbar actions, clipboard paste, and egui `TextEdit` input.
- [ ] Remove or avoid reintroducing special Wispr Flow UI toggles.
- [ ] Design a `StackerInputEngine` boundary owned by the Stacker surface/state, not by generic app-shell event code.
...
- [ ] Normalize CRLF and bare CR input before insertion.
- [ ] Preserve cursor and selection behavior through formatting toolbar actions.
- [ ] Fix Backspace inserting question marks by making Backspace/Delete command operations, never committed text payloads.
...
- [ ] Add unit tests for text insertion, selection replacement, newline normalization, and Backspace/Delete handling.
